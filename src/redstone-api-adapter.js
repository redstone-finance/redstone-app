...
export default {
  getPrice: async function (symbol, opts) {
    if (opts.provider === 'coingecko') {
      ...
      return await redstone.query().symbol(symbol).latest().exec({ defaultProvider: opts.provider })
    }
  },
  getHistoricalPrice: async function (symbol, opts) {
    if (opts.provider === 'coingecko') {
      ...
      const url = `${COINGECKO_URL}/coins/${coingeckoId(
        symbol
      )}/market_chart/range?from=${from}&to=${to}&precision=2&vs_currency=${CURRENCY}`
      const geckoResult = (await axios.get(url)).data

      return geckoResult.prices.reverse().map((p) => priceData(symbol, opts.provider, p[1], p[0]))
    } else {
      return (await redstone.getHistoricalPrice(symbol, fixHistoricalRedstoneProvider(opts))).map(
        (priceData) => {
  ...
  query: async function (provider, symbol, daysCount) {
    if (provider === 'coingecko') {
      const url = `${COINGECKO_URL}/coins/${coingeckoId(
        symbol
      )}/market_chart?days=${daysCount}&precision=2&vs_currency=${CURRENCY}`
      const geckoResult = (await axios.get(url)).data
